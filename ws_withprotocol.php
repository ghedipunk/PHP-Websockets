...
class echoServer extends core_websockets {
  ...
  // protocol_[echobot,broadcasting,json] 
  // extensions_ none implemented yet
  // toolbox_[yourcode] 
  use eventloop_libev,protocol_broadcasting,protocol_echobot;
  ...
  protected function onmessage (&$user, $message) {
    if ($user->headers['protocol']) {
      switch($user->headers['protocol']) {
        case 'json': $parsed = $this->json_decode($message); // example of parsing method to be process later
          break;
      ...
      $this->send($user,$this->json_encode($parsed)); 
    }
    else { 
